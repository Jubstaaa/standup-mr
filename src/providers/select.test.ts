...
describe('chooseKind', () => {
    it('obeys the explicit flag', () => {
        expect(chooseKind({ provider: 'github', env: {}, probe: NONE })).toBe(
            'github'
        )
        expect(chooseKind({ provider: 'gitlab', env: {}, probe: NONE })).toBe(
            'gitlab'
        )
    })
    ...
    it('obeys the environment variable when no flag is given', () => {
        expect(
            chooseKind({ env: { STANDUP_PROVIDER: 'github' }, probe: NONE })
        ).toBe('github')
    })

    it('rejects an unknown provider name', () => {
        expect(() =>
            chooseKind({ provider: 'bitbucket', env: {}, probe: NONE })
        ).toThrow(/bitbucket/)
    })

    it('recognises the two public hosts', () => {
        expect(chooseKind({ host: 'github.com', env: {}, probe: NONE })).toBe(
            'github'
        )
        expect(chooseKind({ host: 'GitLab.com', env: {}, probe: NONE })).toBe(
            'gitlab'
        )
    })
    ...
    it('recognises a self-hosted host by name, so existing invocations keep working', () => {
        expect(
            chooseKind({ host: 'gitlab.example.com', env: {}, probe: NONE })
        ).toBe('gitlab')
        expect(
            chooseKind({ host: 'github.acme.com', env: {}, probe: NONE })
        ).toBe('github')
    })
    ...
    it('asks for --provider when the host name says nothing', () => {
        expect(() =>
            chooseKind({ host: 'git.acme.com', env: {}, probe: NONE })
        ).toThrow(/--provider/)
    })
    ...
    it('uses whichever environment pair is configured', () => {
        expect(chooseKind({ env: { GITHUB_TOKEN: 'ghp' }, probe: NONE })).toBe(
            'github'
        )
        expect(chooseKind({ env: { GITLAB_HOST: 'h' }, probe: NONE })).toBe(
            'gitlab'
        )
    })
