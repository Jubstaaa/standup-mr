...
describe('normalizeChecks', () => {
    ...
    it('reports failed and keeps the failing check id', () => {
        ...
        expect(normalizeChecks(runs)).toEqual({
            pipeline: 'failed',
            pipelineId: 42,
        })
    })
    ...
    it('treats a timeout as failed', () => {
        expect(
            normalizeChecks([check({ conclusion: 'timed_out' })]).pipeline
        ).toBe('failed')
    })

    it('treats a startup failure as failed', () => {
        expect(
            normalizeChecks([check({ conclusion: 'startup_failure' })]).pipeline
        ).toBe('failed')
    })

    it('treats a required manual step as failed, because it blocks the merge', () => {
        expect(
            normalizeChecks([check({ conclusion: 'action_required' })]).pipeline
        ).toBe('failed')
    })
...
describe('countChangesRequested', () => {
    it('counts one reviewer who asked for changes', () => {
        expect(countChangesRequested([review('a', 'CHANGES_REQUESTED')])).toBe(
            1
        )
    })
